...
<div className={styles.dishes_container}>
  ...
  const highestVotedDish = meal_type[Object.keys(meal_type)].reduce((prev, current) => {
    return (global_votes[current.dish_id]?.votes || 0) > (global_votes[prev?.dish_id]?.votes || 0) ? current : prev;
  }, null);
