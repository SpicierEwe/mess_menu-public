                  ...
                  <div key={meal_type_index}>
                    ...
                    <div className={styles.dishes_container}>
                      {meal_type[Object.keys(meal_type)].map((dish, index) => {
                        // Find the dish with the highest votes
                        const topDish = meal_type[
                          Object.keys(meal_type)
                        ].reduce((acc, curr) => {
                          return (global_votes[curr.dish_id]?.votes || 0) >
                            (global_votes[acc.dish_id]?.votes || 0)
                            ? curr
                            : acc;
                        }, dish);

                        // Check if the current dish is the top dish
                        const isTopDish = dish.dish_id === topDish.dish_id;
