...
function DashboardComponent() {
  ...
  function fetch_menu_data() {
    ...
    function fetch_user_personal_voted_dishes() {
      const userDbRef = ref(
        database,
      ...
      get(userDbRef).then((snapshot) => {
        if (snapshot.exists()) {
          ...
          for (const key in personal_voted_dishes) {
            // console.log(personal_voted_dishes[key]["dish_id"]);
            const voted_dish_id = personal_voted_dishes[key]["dish_id"];

            for (const x in meal_table_weekday_data) {
              for (const y in meal_table_weekday_data[x][
                Object.keys(meal_table_weekday_data[x])
              ]) {
                if (
                  meal_table_weekday_data[x][
                    Object.keys(meal_table_weekday_data[x])
                  ][y]["dish_id"] == voted_dish_id
                ) {
                  meal_table_weekday_data[x][
                    Object.keys(meal_table_weekday_data[x])
                  ][y]["voted"] = true;
    ...
    fetch_user_personal_voted_dishes();
  }
  ...
  function vote(dish_id, week_day, meal_type) {
    ...
    console.log(
      "This is the dish id: " + dish_id, // dish_id
      "This is the week day: " + week_day, // week_day
      "This is the meal type: " + meal_type // meal_type
    );
