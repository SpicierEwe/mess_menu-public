          ...
          <div className={styles.container_2_sub_main_container}>
            ...
            <div>
              ...
              {meal_table_weekday_data.map((meal_type, meal_type_index) => {
                const meal_type_data = meal_type[Object.keys(meal_type)];
                const votes = global_votes;
                const votes_list = [];

                for (const vote in votes) {
                  // votes_list.push(votes[vote]);
                  console.log("lmksdkksnd");
                  console.log(Object.keys(votes[vote]));
                }

                const t = [];
                for (const dish of meal_type_data) {
                  console.log(dish.dish_id);
                }

                const sortedDishes = meal_type_data;
                return (
