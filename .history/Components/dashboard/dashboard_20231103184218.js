...
function DashboardComponent() {
  ...
  function vote(dish_id) {
    // global votes
    const arrayRef = ref(database, "/global_votes/" + dish_id);

    // IF dish vote DATA DOESNT EXIST
    // Set the value of 'global_votes'
    const newItem = {
    ...
    update(arrayRef, {
      dish_id: dish_id,
      // votes: global_votes.filter((item) => item.dish_id === dish_id)[0]
      //   ? global_votes.filter((item) => item.dish_id === dish_id)[0].votes + 1
      //   : 1,
      votes: global_votes.filter((item) => item.dish_id === dish_id)[0]
        ? global_votes.filter((item) => item.dish_id === dish_id)[0].votes + 1
        : 1,
