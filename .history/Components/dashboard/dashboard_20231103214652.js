...
function DashboardComponent() {
  ...
  function vote(dish_id, week_day, meal_type) {
    ...
    // find if dish already voted

    if (voted_dishes.length != 0) {
      voted_dishes.forEach((dish) => {
        if (dish.dish_id === dish_id) {
          console.log("alrady voted unvoting dish");
          set_voted_dishes(voted_dishes.splice(voted_dishes.indexOf(dish), 1));
          console.log(voted_dishes);
          return;
        } else {
          console.log("voting dish");
