          ...
          <div className={styles.container_2_sub_main_container}>
            ...
            <div>
              ...
              {meal_table_weekday_data.map((meal_type, meal_type_index) => {
                // find the most voted dish
                let high_votes = 0;
                let most_voted_dish_id = "";

                const rearranged = [];

                for (const key in global_votes) {
                  if (global_votes[key].votes > high_votes) {
                    high_votes = global_votes[key].votes;
                    most_voted_dish_id = key;
                  }
                }

                const most_voted_dish = meal_type[
                  Object.keys(meal_type)
                ].filter((dish) => {
                  dish.dish_id === most_voted_dish_id;
                });

                rearranged.unshift(most_voted_dish);
                const rest = meal_type[Object.keys(meal_type)].filter(
                  (dish) => dish.dish_id !== most_voted_dish_id
                );

                meal_type[Object.keys(meal_type)] = [...rearranged, ...rest];

                return (
                  <div key={meal_type_index}>
                    {/* Heading */}
                    <h1>{most_voted_dish_id}</h1>
