...
*/
  ...
  // ==================================================== //
  const sortedDishes = meal_table_weekday_data.map((meal_type) => {
    const highestVotedDishId = getHighestVotedDishId(
      meal_type[Object.keys(meal_type)]
    );
    return [
      highestVotedDishId,
      ...meal_type[Object.keys(meal_type)].filter(
        (dish) => dish.dish_id !== highestVotedDishId
      ),
    ];
  });

  // Function to get the dish with the highest votes
  function getHighestVotedDishId(dishes) {
    let highestVotes = 0;
    let highestVotedDishId = null;
    for (const dish of dishes) {
      if (global_votes[dish.dish_id]?.votes > highestVotes) {
        highestVotes = global_votes[dish.dish_id].votes;
        highestVotedDishId = dish.dish_id;
      }
    }
  ...
  return (
    <div className={styles.main_parent_container}>
          ...
          <div className={styles.container_2_sub_main_container}>
            ...
            <div>
              ...
              {meal_table_weekday_data.map((meal_type, meal_type_index) => {
                let highest_votes = 0;
                let highest_voted_dish_id = "";

                for (const key in global_votes) {
                  if (global_votes[key].votes > highest_votes) {
                    highest_voted_dish_id = key;
                  }
                }

                const rearranged_meal_types_list = [];
                meal_type[Object.keys(meal_type)].filter((dish, index) => {
                  if (dish.dish_id === highest_voted_dish_id) {
                    rearranged_meal_types_list[0] = dish;
                  }
                });
                meal_type[Object.keys(meal_type)].filter((dish, index) => {
                  if (dish.dish_id !== highest_voted_dish_id) {
                    rearranged_meal_types_list.push(dish);
                  }
                });
      ...
      {sortedDishes.map(([highestVotedDishId, ...dishes], meal_type_index) => {
        return (
          <div key={meal_type_index}>
            {/* Heading */}
            <div className={styles.headings}>
              <h3>{Object.keys(dishes[0])}</h3>
              <p>All available dishes</p>
            </div>
            {/* Dishes display */}
            <div className={styles.dishes_container}>
