  ...
  function get_global_votes() {
    ...
    onValue(dbRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        //
        let highest_voted_dish_vote_count = 0;
        let highest_voted_dish_ids = [];
        for (const id in global_votes) {
          if (global_votes[id].votes > highest_voted_dish_vote_count) {
            highest_voted_dish_vote_count = global_votes[id].votes;
            highest_voted_dish_ids.push(id);
          }
        }
        const restored = [];
        meal_table_weekday_data.map((meal_type, meal_type_index) => {
          meal_type[Object.keys(meal_type)].map((dish, dish_index) => {
            if (highest_voted_dish_ids.includes(dish.dish_id)) {
              const modified_dish_data = {
                dish_id: dish.dish_id,
                week_day: selected_week_day.toLowerCase(),
                meal_type: Object.keys(meal_type)[0],
              };
              restored.push(modified_dish_data);
            }
          });
        });
          ...
          <div className={styles.container_2_sub_main_container}>
            ...
            <div>
              ...
              {meal_table_weekday_data.map((meal_type, meal_type_index) => {
                ...
                for (const votes in global_votes) {
                  if (
                    global_votes[votes].votes > highest_voted_dish_vote_count
                  ) {
                    highest_voted_dish_vote_count = global_votes[votes].votes;
                    highest_voted_dish_id = votes;
                  }
                }

                console.log(
                  "id ==== " +
                    highest_voted_dish_id +
                    " " +
                    "dish votes" +
                    highest_voted_dish_vote_count
                );

                const x = meal_type[Object.keys(meal_type)];
                const remain = x.filter(
                  (dish) => dish.dish_id !== highest_voted_dish_id
                );
                          ...
                          <div key={index}>
                            <div
                              ...
                              className={`${
                                ...
                                dish.dish_id === highest_voted_dish_id
