  ...
  function get_global_votes() {
    ...
    onValue(dbRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const highestVotedDish = meal_type[Object.keys(meal_type)].reduce(
          (prev, current) => {
            return (global_votes[current.dish_id]?.votes || 0) >
              (global_votes[prev?.dish_id]?.votes || 0)
              ? current
              : prev;
          },
          null
        ...
        const rearrangedMealType = meal_type[Object.keys(meal_type)].map(
          (dish) => {
            if (dish === highestVotedDish) {
              return dish; // Keep the highest-voted dish at its original position
            } else {
              return null;
            }
          }
        ...
        // Filter out the null values (i.e., non-highest-voted dishes)
        const nonHighestVotedDishes = meal_type[Object.keys(meal_type)].filter(
          (dish) => dish !== highestVotedDish
        );

        // Concatenate the highest-voted dish and the non-highest-voted dishes
        const combinedDishes = [
          ...rearrangedMealType,
          ...nonHighestVotedDishes,
        ];

        // Now, combinedDishes contains the highest-voted dish at the front and the rest in their original order
      } else {
        console.log("No data available");
      }
    });
