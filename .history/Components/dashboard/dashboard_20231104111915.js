  ...
  function fetch_menu_data() {
    ...
    get(dbRef).then((snapshot) => {
      if (snapshot.exists()) {
        ...
        set_meal_table_weekday_data(modified_data[selected_week_day]);
    ...
    function get_user_personal_votes() {
      // Set up a listener to get realtime updates
      get(userDbRef, (snapshot) => {
        if (snapshot.exists()) {
          console.log("inditial user votes data from db = " + snapshot.val());
        } else {
          console.log("No initial voted user data available");
        }
      });
    }

    get_user_personal_votes();
  }
