...
function LeaderboardComponent() {
  ...
  function xx(week_day_name) {
    ...
    return meal_menu_data[week_day_name]?.map((data, index) => {
      const dish_data_list = data[Object.keys(data)];
      return dish_data_list.map((dish_data, index) => {
        const meal_type = Object.keys(dish_data)[0];
      return  for (const voted_id in global_votes) {
          if (voted_id === dish_data[meal_type].id) {
            highest_votes.push(dish_data[meal_type]);
          }
        }
      });
    });
