...
function LeaderboardComponent() {
  ...
  function xx(week_day_name) {
    function xx(week_day_name) {
        ...
        dish_data_list.map((dish_data, index) => {
          ...
          for (const voted_id in global_votes) {
            if (voted_id === dish_data.dish_id) {
              if (
                global_votes[voted_id].votes >
                (highest_voted_dishes[meal_type]?.votes || 0)
              ) {
                highest_voted_dishes[meal_type] = {
                  meal_type: meal_type,
      ...
      console.log(highest_voted_dishes);
              ...
              {week_days.map((week_day, index) => (
                <tr key={index}>
                  ...
                  {xx(week_day.toLowerCase()).map((dish_data, index) => {
                    return meal_types_list.map((meal_type, index) => {
                      if (dish_data.meal_type === meal_type.toLowerCase()) {
                        return (
                          <td key={index}>{dish_data.dish_data.dish_name}</td>
                        );
                      } else {
