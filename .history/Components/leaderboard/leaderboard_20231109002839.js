    ...
    get(dbRef).then((snapshot) => {
      if (snapshot.exists()) {
        ...
        week_days.map((week_day, index) => {
          data[week_day.toLowerCase()].map((meal_type_data, index) => {
            const week_day_name = Object.keys(meal_type_data);
            const meal_type_name = Object.values(meal_type_data);

            global_votes.map((global_vote, index) => {
              const voted_dish_id = Object.keys(global_vote);

              // if(voted_dish_id === )
              meal_type_data.map((dish, index) => {
                dish.map((dish_data, index) => {
                  if (dish_data.dish_id === voted_dish_id) {
                    console.log("this is the dish id", dish_data.dish_id);
                    console.log("this is the voted dish id", voted_dish_id);
                    console.log("this is the dish data", dish_data);
                    console.log("this is the voted dish data", global_vote);
                  }
                });
              });
            });

            set_meal_menu_data((prev) => [...prev, meal_type_data]);
          });
        });
      } else {
