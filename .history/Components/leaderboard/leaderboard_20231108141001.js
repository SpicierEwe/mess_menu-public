          ...
          {week_days.map((week_day, index) => (
            ...
            <tr key={index}>
              ...
              {meal_types_list.map((meal_type, index) => {
                ...
                if (weekday_data) {
                  weekday_data.forEach((meal_type_data) => {
                    const meal_type_name = Object.keys(meal_type_data)[0];
                    const meal_type_dishes = meal_type_data[meal_type_name];

                    if (meal_type_name === meal_type) {
                      meal_type_dishes.forEach((dish) => {
                ...
                return <td key={index}>{dishIds.join(", ")}</td>;
              })}
            </tr>
          ))}
        </tbody>
