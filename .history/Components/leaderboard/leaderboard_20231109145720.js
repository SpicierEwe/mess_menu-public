...
function LeaderboardComponent() {
  ...
  function xx(dish_data_list) {
    const highest_votes = [];

    meal_menu_data.map((data, index) => {
      const dish_data_list = data[Object.keys(data)];
      dish_data_list.map((dish_data, index) => {
        const dish_name = Object.keys(dish_data)[0];
        const dish_votes = dish_data[dish_name];
        highest_votes.push(dish_votes);
      });
    });
  }
