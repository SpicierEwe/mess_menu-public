          ...
          {week_days.map((week_day_name, index) => {
            return (
              <div key={index}>
                <p>{week_day_name}</p>
                <div>
                  {meal_menu_data[week_day_name.toLowerCase()]?.map(
                    (data, index) => {
                      const dish_data_lost = data[Object.keys(data)];
                      return (
                        <p key={index}>
                          {/* {data[Object.keys(data)].map((dish_data, index) => {
                            return (
                              <p key={index}>
                                {xx(dish_data.dish_id)
                                  ? JSON.stringify(dish_data)
                                  : "null"}
                              </p>
                            );
                          })} */}
