              ...
              {week_days.map((week_day, index) => {
                return (
                  <tr key={index}>
                    ...
                    {meal_types_list.map((meal_type, index) => (
                      <td key={index}>
                        {meal_menu_data[week_day.toLowerCase()]?.map(
                          (meal_type_data, index) => {
                            return (
                              <div key={index}>
                                {meal_type_data[
                                  Object.keys(meal_type_data)
                                ].map((dish, index) => {
                                  const data_meal_type_name =
                                    Object.keys(meal_type_data)[0];
                                  const data_week_day_name =
                                    Object.keys(meal_menu_data)[0];

                                  global_votes?.map((votes, index) => {
                                    const voted_dish_id = Object.keys(votes)[0];

                                    if (dish.dish_id == voted_dish_id) {
                                      return (
                                        <div key={index}>
                                          {dish.dish_name} -
                                          {votes[voted_dish_id].votes}
                                        </div>
                                      );
                                    }
                                  });
